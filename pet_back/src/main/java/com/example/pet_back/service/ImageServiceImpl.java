...
//    }
    ...
    public String saveImage(MultipartFile file)  {
        ...
        String directory = fileUploadProperties.getStaticPath(); // 디렉토리
        //값 제대로 받아왔는지 체크
        ...
        String fullPath = directory + File.separator + saveFileName;

        log.info("저장 경로 => " + directory);

        try {
            File saveDir = new File(directory); //저장할 디렉토리 생성
            //만얃 없다면 디렉토리를 생성한다.
    ...
    public ResponseEntity<?> memberUploadImage(Long id, MultipartFile file) {
        ...
        String imageURL = fileUploadProperties.getUserUrl() + saveFileName;
    ...
    public ApiResponse bannerInsert(MultipartFile file,int position) {
        ...
        return new ApiResponse(true,position+"번째 배너에 추가 되었습니다.");
    }

    // 글 삭제 시 이미지 파일 실제 삭제
    public void deleteImageFile(String fileName){
        String uploadDir = fileUploadProperties.getBoardPath(); // getRealPath에서 교체함
