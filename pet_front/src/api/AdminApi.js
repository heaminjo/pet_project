...
const AdminApi = {
  ...
  getUserData: async (id) => {
    const result = await instance.get(`${KH_DOMAIN}/admin/user/detail?id=${id}`);
    return result.data;
  },
  ...
  getOrderStatistics: async (date) => {
    const result = await instance.get(`${KH_DOMAIN}/admin/statistics/order?date=${date}`);
    return result.data;
  },

  //통계용 상품 랭크 리스트
  getGoodsRank: async () => {
    const result = await instance.get(`${KH_DOMAIN}/admin/statistics/goods/rank`);
    return result.data;
  },
};
