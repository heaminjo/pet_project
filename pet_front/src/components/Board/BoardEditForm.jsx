      ...
      <div className='boardInsertFormContainer'>
        ...
        <form onSubmit={handleSubmit}>
          <div className='titleRow'>
            ...
            <input type='text' id='title' name='title' className='titleInput' value={title} onChange={(e) => setTitle(e.target.value)} required />
          </div>
          <div className='categoryRow'>
            <label htmlFor='category' className='categoryLabel'>
              게시판
            </label>
            <select id='category' name='category' className='categorySelect' value={category || ''} required disabled>
              {role === 'ROLE_ADMIN' && <option value='notice'>공지사항</option>}
          ...
          <div className='contentRow'>
            ...
            <textarea id='content' name='content' className='contentTextarea' value={content} onChange={(e) => setContent(e.target.value)} required></textarea>
          </div>
          <div className='fileRow'>
            <div className='uploadBox'>
              ...
              <input type='file' id='file' accept='image/*,.pdf,.doc,.docx,.xls,.xlsx,.zip,.hwp,.ppt,.pptx' multiple onChange={handleNewFileChange} style={{ display: 'none' }} />
              <span className='fileNameText'>{newFiles.length > 0 ? newFiles.map((f) => f.name).join(', ') : '파일을 선택하세요.'}</span>
            </div>
              ...
              <div className='imagePreview'>
                {existingFiles.map((file, idx) => (
                  <div className='imagePreviewBox' key={file.file_name}>
                    {file.file_type && file.file_type.startsWith('image/') ? <img src={`${KH_DOMAIN}/resources/webapp/userImages/${file.file_name}`} alt={file.origin_name} width={200} /> : <span>{file.origin_name}</span>}
                    <button type='button' className='removeFileBtn' onClick={() => handleExistingFileRemove(idx)}>
                      x
                    </button>
                  </div>
                ))}
              ...
              <div className='imagePreview'>
                {newImagePreviews.map((src, idx) => (
                  <div className='imagePreviewBox' key={idx}>
                    ...
                    <button type='button' className='removeFileBtn' onClick={() => handleNewFileRemove(idx)}>
                      x
                    </button>
                  </div>
                ))}
